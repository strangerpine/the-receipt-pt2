    ...
    <div
      ...
      style={{
        ...
        boxShadow: hovered && hover
          ? '0 12px 40px rgba(200,169,126,0.08)'
    ...
    <section id="home" style={{
      ...
      <div style={{
        ...
        <h1 style={{
          ...
          Four Years of<br />
          <span style={{ color: T.accent, fontStyle: 'italic' }}>Becoming</span>
        </h1>

        <p style={{
          maxWidth: 560, margin: '0 auto 2.5rem', fontSize: 'clamp(1rem, 2vw, 1.15rem)',
          color: T.textDim, lineHeight: 1.8,
          opacity: loaded ? 1 : 0,
          transition: 'opacity 1s ease 0.6s',
        }}>
          A reflective journey through inquiry, growth, and transformation
  ...
  <Section id="about">
    ...
    <div style={{
      ...
      <div style={{
        ...
        {[
          { icon: '🔬', label: 'Inquiry', desc: 'Evidence-based thinking & research methods' },
          { icon: '🤝', label: 'Community', desc: 'Service, advocacy & collaborative practice' },
          { icon: '🌱', label: 'Growth', desc: 'Personal development & reflective learning' },
          { icon: '💡', label: 'Innovation', desc: 'Creative problem-solving & design thinking' },
        ].map(item => (
          <Card key={item.label} style={{ textAlign: 'center', padding: '1.5rem' }}>
            <div style={{ fontSize: '1.8rem', marginBottom: '0.75rem' }}>{item.icon}</div>
            <h4 style={{
              fontFamily: T.serif, color: T.white, fontSize: '1rem',
              marginBottom: '0.5rem',
            }}>{item.label}</h4>
